...
export default function CareerPage() {
  const judicialCareer = CAREER_HISTORY.filter((c) => c.category === "judicial");
  const academicCareer = CAREER_HISTORY.filter((c) => c.category === "academic");
  ...
  return (
    <main className="bg-white text-[#17201f]">
      ...
      <PageHero
        ...
        title={
          <>
            A Life in <em className="text-[#e6c66a] not-italic">Courts, Classrooms,</em> &amp; Public Service
          </>
        }
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          ...
          <div>
            ...
            <p className="mt-2 text-xs leading-relaxed text-[#66706b]">
              J.S.D. &amp; LL.M. (New York University); Certificate in International Law (Hague Academy);
              M.C.J. (Howard University); LL.B. (University of East Africa, Dar Es Salaam).
            </p>
          </div>
        </div>
      </section>
      ...
      >
        <div className="max-w-5xl">
          ...
          <p className="mt-4 max-w-2xl text-base text-[#66706b]">
            Decisions rendered and appeals presided over at the International Criminal Court and the Special Tribunal for Lebanon.
          </p>
      ...
      >
        <div className="max-w-5xl">
          ...
          <p className="mt-4 max-w-2xl text-base text-[#66706b]">
            Over three decades of university instruction in Public International Law, Criminal Law, Evidence, and Human Rights across Botswana, Canada, and Uganda.
          </p>
          ...
          <div className="mt-12 border-t border-[#d9d1c4]">
            {unAndPractice.map((entry, idx) => (
              <TimelineItem
                ...
                location={entry.location}
                description={entry.description}
                badge={entry.category === "un" ? "United Nations" : "Law Practice"}
              />
            ))}
          </div>
        </div>
      ...
      >
        <div className="max-w-5xl">
          ...
          <p className="mt-4 max-w-2xl text-base text-[#66706b]">
            High-level trial observation, human rights field investigations, and treaty negotiation advisory for African governments and international organizations.
          </p>
            ...
            {PROFESSIONAL_ACTIVITIES.map((act, idx) => (
              ...
              >
                <div className="flex items-center justify-between text-xs">
                  <span className="font-semibold text-[#c64e38]">{act.year}</span>
      ...
      <section className="px-6 py-16 sm:px-10 lg:px-[5.5vw] lg:py-24">
        <div className="max-w-5xl">
          ...
          <p className="mt-4 max-w-2xl text-base text-[#66706b]">
            Serving as external examiner, doctorate committee evaluator, and promotions assessor for major African and European universities.
          </p>
            ...
            <table className="w-full border-collapse text-left text-xs">
              <thead>
                <tr className="border-b-2 border-[#17201f] text-[#17201f]">
                  ...
                  <th className="py-3 px-4 font-semibold uppercase">Institution</th>
                  <th className="py-3 px-4 font-semibold uppercase">Location</th>
                  <th className="py-3 pl-4 font-semibold uppercase">Role &amp; Details</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#d9d1c4]">
                {EXTERNAL_EXAMINATIONS.map((exam, idx) => (
                  <tr key={idx} className="hover:bg-[#faf6ef]">
                    <td className="py-3 pr-4 font-medium text-[#c64e38]">{exam.period}</td>
                    <td className="py-3 px-4 font-semibold text-[#17201f]">{exam.institution}</td>
                    <td className="py-3 px-4 text-[#66706b]">{exam.location}</td>
          ...
          <div className="mt-10 grid gap-4 sm:grid-cols-2">
            {MEMBERSHIPS.map((mem, idx) => (
              ...
              >
                ...
                <p className="mt-1 text-xs text-[#b8c5c1]">{mem.organization}</p>
              </div>
            ))}
          </div>

          <div className="mt-14 border-t border-[#fff8ed]/20 pt-10">
            <h3 className="font-serif text-2xl text-[#fff8ed]">Honours &amp; Fellowships</h3>
            <div className="mt-6 grid gap-6 sm:grid-cols-2">
              {AWARDS.map((award, idx) => (
                <div key={idx} className="rounded border border-[#fff8ed]/20 bg-[#24312f]/40 p-5">
                  <span className="text-xs font-semibold text-[#e6c66a]">{award.year}</span>
                  <h4 className="mt-1 font-serif text-lg text-[#fff8ed]">{award.title}</h4>
                  <p className="mt-1 text-xs font-medium text-[#d8e0dc]">
                    {award.institution} · {award.location}
                  </p>
                  {award.description && (
                    <p className="mt-2 text-xs text-[#b8c5c1]">{award.description}</p>
                  )}
                </div>
              ))}
            </div>
      ...
      <section className="bg-white px-6 py-16 text-center sm:px-10 lg:px-[5.5vw] lg:py-20">
        ...
        <p className="mx-auto mt-4 max-w-xl text-sm leading-relaxed text-[#66706b]">
          Judge Nsereko participates in select lectures, conferences, and comparative criminal justice panels.
        </p>
